...
def bm25_retrieval(k, question):
    ...
    docs =  [chunks[i] for i in top_k]
    
    return normalize_doc(docs, "reranked")
...
# Main function
def run_rag_pipeline(retriever, k, answer_model, answer_type, reranker, question, need_rerank, embedding):
    ...
    if need_rerank:
        ...
        docs = normalize_doc(docs, "reranked")

    try:
        retrieved_context = "\n\n".join(d.page_content for d in docs)
    except Exception as e:
        docs = normalize_doc(docs, "error")
        retrieved_context = "\n\n".join(d.page_content for d in docs)
    
    if answer_type == "qa":
        ans = answer_model(question = question, context = retrieved_context)["answer"]

    elif answer_type == "llm":
